...
public class PatchInactivateCouponByCouponId_v1Steps {
    ...
    public void possuoDadosParaCouponServiceV1PatchActivateCouponByCouponId(Map<String, String> arg1) {
        BuilderRequest.path.accept(Util.rota.apply(CouponV1.PATCH_INACTIVATE_COUPON_BY_ID), arg1);
    }
    ...
    public void sistemaProcessaCouponServiceV1PatchActivateCouponByCouponId(DataTable arg1) throws Throwable {
        ResponseValidator.validaResponse(arg1
                , Cache.chamadas.get(Util.rota.apply(CouponV1.PATCH_INACTIVATE_COUPON_BY_ID)).getServiceResponse()
                , String.class
        );
    }

    @E("valido CouponService - v1 - PatchInactivateCouponByCouponId {string}")
    public void validoCouponServiceV1PatchActivateCouponByCouponId(String mensagemPrevista) {
        int statusCode = Cache.chamadas.get(Util.rota.apply(CouponV1.PATCH_INACTIVATE_COUPON_BY_ID)).getServiceResponse().getStatusCode();
        assumingThat(statusCode == 204, () -> {
            String response = Cache.chamadas.get(Util.rota.apply(CouponV1.PATCH_INACTIVATE_COUPON_BY_ID)).getServiceResponse().getObjectBody(String.class);
            Assert.assertTrue(response == null);
        });
        assumingThat(statusCode == 404, () -> {
            String mensagemRetorno = Cache.chamadas.get(Util.rota.apply(CouponV1.PATCH_INACTIVATE_COUPON_BY_ID)).getServiceResponse().getBody();
